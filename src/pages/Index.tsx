  ...
  const handlePhoneSubmit = async (phone: string, preferredDay?: string, preferredTime?: string, recaptchaToken?: string | null) => {
    ...
    try {
      ...
      const queryParams = new URLSearchParams({
        ...
        landing_variant: 'GeologyOnboarding',
        class_id: classId,
      ...
      const redirectUrl = `${config.redirectBaseUrl}/thank-you-landing?name=${encodeURIComponent(finalAnswers.name || '')}&email=${finalAnswers.email}&source=${encodeURIComponent(trackingParams.source)}${trackingParams.referrerId ? `&referrerId=${encodeURIComponent(trackingParams.referrerId)}` : ''}${finalAnswers.q1 ? `&how_soon=${encodeURIComponent(mapHowSoon(finalAnswers.q1))}` : ''}${finalAnswers.q2 ? `&preferred_topics=${encodeURIComponent(mapSchoolingMode(finalAnswers.q2))}` : ''}${phone ? `&phone_number=${encodeURIComponent(phone)}` : ''}${preferredDay ? `&preferred_day=${encodeURIComponent(preferredDay)}` : ''}${preferredTime ? `&preferred_time=${encodeURIComponent(preferredTime)}` : ''}&landing_variant=GeologyOnboarding&class_id=${classId}&redirectTo=${redirectToUrl}&landing_secret=ca_landing_2025_3xD9pQ1Z${recaptchaToken ? `&recaptchaToken=${encodeURIComponent(recaptchaToken)}` : ''}`;
